...
var endState = {
	create: function() {
		...
		//First message
		var wonOrDie = game.add.text(110, 40, (this.die) ? 'You die...' : 'You won !!!', { font: '50px Indie Flower', fill: '#ffffff' });
		...
		//score
		var score = game.add.text(125, 85, 'Score: '+ points, { font: '35px Indie Flower', fill: '#ffffff' });
		...
		var detailsToDestroy = game.add.text(160, 130, details, { font: '25px Indie Flower', fill: '#ffffff' });
		...
		menuButton.events.onInputUp.add(function(){game.state.start('menu');}, this);

		var menuText = game.add.text(menuButton.x + 90, menuButton.y + 20, 'Menu', { font: '35px Indie Flower', fill: '#ffffff' });
		buttons.add(menuText);

		buttons.tween = game.add.tween(buttons).to({y: 300},2000,Phaser.Easing.Linear.None, false);
		...
		//More stats ?
		var more = game.add.text(220, 240,'More stats ?', { font: '25px Indie Flower', fill: '#7AE60D' });
		more.inputEnabled = true;
		more.input.useHandCursor = true;
		more.events.onInputUp.add(function(){buttons.tween.start();}, this);
		...
		waveStatsBox.tween = game.add.tween(waveStatsBox).to({alpha: 0.9},2000,Phaser.Easing.Linear.None, false);
		...
		//Head of waveStats
		var headStats = game.add.text(125, 355, ' Life lost per wave:', { font: '35px Indie Flower', fill: '#ffffff' });
		headStats.alpha = 0;
		headStats.tween = game.add.tween(headStats).to({alpha: 1},2000,Phaser.Easing.Linear.None, false);
